...
call ale#Set('typescript_tslint_project_path', '')
...
function! ale_linters#typescript#tslint#GetCommand(buffer) abort
    ...
    let l:tslint_project_path = ale#path#ResolveLocalPath(
    \   a:buffer,
    \   'tsconfig.json',
    \   ale#Var(a:buffer, 'typescript_tslint_project_path')
    \)
    let l:tslint_project_option = !empty(l:tslint_project_path)
    \   ? ' -p ' . ale#Escape(l:tslint_project_path)
    \   : ''
